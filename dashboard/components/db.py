...
def inventory_list(where_clause=None, params=()):
    ...
    if where_clause:
        sql = base_sql + " WHERE " + where_clause + " ORDER BY p.name, w.name;"
        curr.execute(sql, params)
    else:
        sql = base_sql + " ORDER BY p.name, w.name;"
        curr.execute(sql)
...
def out_of_stock_products():
    return inventory_list("i.stock_level = 0")


def low_stock_products(threshold=50):
    return inventory_list("i.stock_level < %s", (threshold,))


def count_low_stock_products(threshold=50):
    con = get_connection()
    curr = con.cursor()
    curr.execute("""
        SELECT COUNT(*)
...
def worst_on_time_delivery_rate_per_supplier():
    con = get_connection()
    curr = con.cursor()
    curr.execute("""
        SELECT s.name,
